...
pub enum GCodeCommand {
    G0 { x: Option<f64>, y: Option<f64>, z: Option<f64>, e: Option<f64> }, // Rapid move
    G1 { x: Option<f64>, y: Option<f64>, z: Option<f64>, e: Option<f64>, f: Option<f64> }, // Linear move
    G28, // Home
    M104 { s: f64 }, // Set temperature
    M109 { s: f64 }, // Wait for temperature
    M300 { s: f64, p: f64 }, // Pneumatic pressure control
    M301 { t: f64, r: f64 }, // Thermal profiling
    M302 { u: f64, t: f64 }, // UV curing
    ...
    pub fn add(&mut self, cmd: GCodeCommand) {
        let line = match cmd {
            GCodeCommand::G0 { x, y, z, e } => {
                ...
                if let Some(v) = x { parts.push(format!("X{:.4}", v)); }
                if let Some(v) = y { parts.push(format!("Y{:.4}", v)); }
                if let Some(v) = z { parts.push(format!("Z{:.4}", v)); }
                if let Some(v) = e { parts.push(format!("E{:.6}", v)); }
                parts.join(" ")
            }
            GCodeCommand::G1 { x, y, z, e, f } => {
                ...
                if let Some(v) = z { parts.push(format!("Z{:.4}", v)); }
                if let Some(v) = e { parts.push(format!("E{:.6}", v)); }
                if let Some(v) = f { parts.push(format!("F{:.0}", v)); }
                parts.join(" ")
            }
